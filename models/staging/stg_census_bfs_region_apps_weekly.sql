...
-- Census BFS weekly business applications by Census region (4 regions:
-- Northeast, Midwest, South, West). 1:1 grain with source — natural key
-- is (year, week, region). All metrics are NSA.

with source as (
    select * from {{ source('raw', 'census_bfs_region_apps_weekly') }}
),
